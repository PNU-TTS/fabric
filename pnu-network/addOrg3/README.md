...
You first need to run `./network.sh up createChannel` in the `pnu-network` directory before you can run the `addOrg3.sh` script.
...
If you used `network.sh` to create a channel other than the default `rec-trade-channel`, you need pass that name to the `addorg3.sh` script.
```
