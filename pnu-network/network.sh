...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the pnucc chaincode on the channel"
  echo "      - 'down' - clear the network with docker-compose down"
  echo "      - 'restart' - restart the network"
  echo
  echo "    Flags:"
  echo "    -ca <use CAs> -  create Certificate Authorities to generate the crypto material"
  echo "    -c <channel name> - channel name to use (defaults to \"rec-trade-channel\")"
  ...
  echo
  echo " Examples:"
  echo "  network.sh up createChannel -ca -c rec-trade-channel -s couchdb -i 2.0.0"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create ManagementPeerOrg Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-management-peer-org.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create RecClientPeerOrg Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-rec-client-peer-org.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    createManagementPeerOrg

    echo "##########################################################"
    echo "############ Create RecClientPeerOrg Identities ######################"
    echo "##########################################################"

    createRecClientPeerOrg
  ...
  echo
  echo "Generate CCP files for ManagementPeerOrg and RecClientPeerOrg"
...
# file for creating the network is "docker-compose-pnu-net.yaml" in the ``docker``
...
## call the script to join create the channel and join the peers of management-peer-org and rec-client-peer-org
...
function networkDown() {
  # stop org3 containers also in addition to management-peer-org and rec-client-peer-org, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/management-peer-org/msp organizations/fabric-ca/management-peer-org/tls-cert.pem organizations/fabric-ca/management-peer-org/ca-cert.pem organizations/fabric-ca/management-peer-org/IssuerPublicKey organizations/fabric-ca/management-peer-org/IssuerRevocationPublicKey organizations/fabric-ca/management-peer-org/fabric-ca-server.db
    rm -rf organizations/fabric-ca/rec-client-peer-org/msp organizations/fabric-ca/rec-client-peer-org/tls-cert.pem organizations/fabric-ca/rec-client-peer-org/ca-cert.pem organizations/fabric-ca/rec-client-peer-org/IssuerPublicKey organizations/fabric-ca/rec-client-peer-org/IssuerRevocationPublicKey organizations/fabric-ca/rec-client-peer-org/fabric-ca-server.db
    ...
    rm -rf channel-artifacts log.txt pnucc.tar.gz pnucc
...
CLI_DELAY=3
# channel name defaults to "rec-trade-channel"
CHANNEL_NAME="rec-trade-channel"
# use this as the default docker-compose yaml definition
COMPOSE_FILE_BASE=docker/docker-compose-pnu-net.yaml
